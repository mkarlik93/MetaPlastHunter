...
#TARA MODULE
    def tara_singletons(self):
        df = load_tara_data()
        deduplicated = dataframe_deduplication(df)
        return deduplicated

    def show_df_headers(self,df):
        headers = list(df.columns.values)
        return headers

    def tara_oceans_lon_lat(self, df):
    ...
    def get_tara_station_list(self):
        df = load_tara_data()
...
#Popracowac nad ta czescia
    def draw_map(self,lon, lat,reads):
...
#for now CANT be ZERO
    ...
    def vizualization_with_tara(self, df):
