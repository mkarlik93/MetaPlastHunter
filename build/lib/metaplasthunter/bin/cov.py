...
class Coverage_utillities:
    ...
    def analyze_emp_genome_list(self):
        ...
        genome_list = [seq.id for seq in (SeqIO.parse(self.database,"fasta"))]
        empirical_keys = self.e_coverage_treshold_ditionary.keys()
        return  list(filter(lambda x: x not in empirical_keys, genome_list))

    def create_reads(self,genome_to_analyze):

        """ Random reads generator of length 100 and isert size between 100 - 300 """

        logger.info("     Read creating " )
        command="%srandomreads.sh ref=%s.fa coverage=2 paired=t out1=%s_1.fq out2=%s_2.fq"  % (genome_to_analyze)
    ...
    def cleaning(self,project_name):
        ...
        """ Removing data files and already used artificial reads """

        command = "rm -r %s" % (project_name)
        command_2 = "rm %s_1.fq %s_2.fq"  % (project_name, project_name)
        process_1 = Popen(command, stdout=PIPE, stderr=PIPE)
        stdout, stderr = process.communicate()
        process_2 = Popen(command_2, stdout=PIPE, stderr=PIPE)
        stdout, stderr = process.communicate()

    def run_MetaPlastHunter(self,genome_to_analyze):
        logger.info("   Creating artificial reads" )
        create_reads(genome_to_analyze)
        logger.info("   Estimating coverage treshold" )
        command="MetaPlastHunter --in1 %s_1.fq --in2 %s_2.fq -C --out --settings %s"  % (xxx)
        command = command.split(" ")
        process = Popen(command, stdout=PIPE, stderr=PIPE)
        stdout, stderr = process.communicate()
        old_dir = os.getcwd()
    ...
    def add_empirical_treshold(self):
        ...
        if self.e_coverage_treshold  == "" :
            ...
            if genome_records > 1:
                ...
                with open(self.add_empirical_treshold,"rw") as f:
