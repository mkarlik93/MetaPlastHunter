...
#TODO complete this function and check

def generates_ani(target_species_genome_list):


    """ Returns average nucleotide identity level of reads of well
     covered genomes (pre assigned at species level)
     min_ at species level """

    #Dictyonary generation

    empty_dictionary = {}
    for genome_name in target_species_genome_list:
        empty_dictionary[genome_name] = []

    samfile_path = glob.glob("*_final_mapped.sam")[0]
        ...
        try:
            if read.is_unmapped:
                ...
                if read.is_secondary:
                    ...
                    if read.reference_name in empty_dictionary.keys:
                        empty_dictionary[read.reference_name].append(float(read.tags[-1][1]))
        except:
            pass

    updated_dictionary = empty_dictionary

    for genome_name in updated_dictionary:
        average_min_identity = numpy.mean(updated_dictionary[genome_name])
        updated_dictionary[genome_name] = average_min_identity
...
class Taxonomic_assignment(object):
    ...
    def absolute_coverage_loading_to_NCBI(self):
        ...
        with open("cov_list.txt", "r") as f:
            return dict(
                [(self.taxid_name_translator(int(self._seqid[line.split(",")[0]])), line.split(",")[1].strip("\n")) for
                 line in f])
    ...
    def empirical_coverage_to_NCBI(self):
        ...
        return dict(
            [(self.taxid_name_translator(int(self._seqid[key])), float(empirical_threshold[key].strip("\n"))) for key in
             empirical_threshold])
    ...
    def mean_coverage_to_NCBI(self):
        ...
        return dict([(self.taxid_name_translator(int(self._seqid[key])), float(self.avg_coverage_dict[key])) for key in
                     self.avg_coverage_dict])
    ...
    def lca_assignment(self):
        ...
        for i in itertools.permutations(cov, 2):
            ...
            if self.get_lineage_without_tree(i[0], ncbi)[-1] not in dict_of_assigned:
                ...
                if len(self.get_lineage_without_tree(dict_of_assigned[self.get_lineage_without_tree(i[0], ncbi)[-1]],
                                                     ncbi)) < len(self.get_lineage_without_tree(lca.taxid, ncbi)):
                    dict_of_assigned[self.get_lineage_without_tree(i[0], ncbi)[-1]] = lca.taxid
    ...
    def lca_assignment_just_taxids(self):
        ...
        if len(cov) >= 1:
            ...
            if len(cov) == 1:
                ...
                logger.info(
                    "It seems that only one cpGenome were detected but with low coverage. Those reads were assigned as Eukaryote")
                ...
                for i in itertools.permutations(cov, 2):
                    ...
                    if i[0] not in dict_of_assigned:
                        ...
                        if len(self.get_lineage_without_tree(dict_of_assigned[i[0]], ncbi)) < len(
                                self.get_lineage_without_tree(lca.taxid, ncbi)):
                            dict_of_assigned[i[0]] = lca.taxid
    ...
    def _ltu_calc_with_taxa_name(self):
        ...
        unique = {}
        lca_assign, almost_full = self.lca_assignment(self._seqid)
    ...
    def sam_parse_experimental(self):
        ...
        assigned_reads = dict(
            [(self._seqid[reference_genome], tmp_dict_of_reads[reference_genome]) for reference_genome in
             tmp_dict_of_reads])
        ...
        taxa_count_dict = dict(
            (str(";".join(self.get_lineage(taxa))), sum(v.values())) for taxa, v in the_cdr_data_structure.items())
            ...
            for i in range(len(lineage) - 1):
                ...
                if lineage[i + 1] not in G.nodes():
                    if lineage[i + 1] in self.dict_ltu:
                        G.add_node(lineage[i + 1], count=int(self.taxa_count_dict[key]),
                                   ltu_genomes=self.dict_ltu[lineage[i + 1]])
            ...
            if 'ltu_genomes' in taxonomic_assignment_graph.nodes[node]:
                ...
                if type(taxonomic_assignment_graph.nodes[node]['ltu_genomes']) is dict:
                    ...
                    try:
                        ...
                        ltu_sum = sum([float(self.absolute_coverage_loaded[x]) for x in
                                       taxonomic_assignment_graph.nodes[node]['ltu_genomes'].keys()])

                        avg_empirical_threshold = relaxation_parameter * numpy.mean(
                            [float(empirical_threshold[x]) for x in
                             taxonomic_assignment_graph.nodes[node]['ltu_genomes'].keys()])
    ...
    def ltu_catch(self):
        ...
        for node in self.analyzed_graf.nodes():
            if self.analyzed_graf.degree[node] == 1 and node != "root":
                ltu_after_min_support.append((nx.shortest_path(self.analyzed_graf, "root", target=node),
                                              self.analyzed_graf.nodes[node]['count']))
            ...
            if 'ltu_genomes' in self.analyzed_graf.nodes[node]:
                ...
                if type(self.analyzed_graf.nodes[node]['ltu_genomes']) is dict and node in probable_taxa:
                    ...
                    try:
                        ...
                        ltu_after_min_support.append((nx.shortest_path(self.analyzed_graf, "root", target=node),
                                                      self.analyzed_graf.nodes[node]['count'], avg_sum))
                    except TypeError:
    ...
    def krona_prunned_avg_coverage(self, project_name):
        ...
        cmd = "ktImportText %s -o %s" % (
        project_name + "_average_coverage_krona.txt", project_name + "_average_coverage_krona.html")
...
class Taxonomic_assignment_Runner:
    ...
    def __init__(self, input, output, settings):
        ...
        try:
            ...
            self.seqidmap = Settings_loader_yaml(self.settings).yaml_handler()["Databases and mapping files"][
                "seqid2taxid.map"]
    ...
    def process(self):
        ...
        lca_postprocess = Taxonomic_assignment(self.project_name, self.lca_threshold, self.seqidmap, self.sam_type,
                                               self.input, self.settings, avg_coverage)
